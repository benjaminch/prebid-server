...
func TestSiteVideo(t *testing.T) {
	...
	_, errs := exchangeBidder.requestBid(context.Background(), ortbRequest, openrtb_ext.BidderRubicon, 1.0, rateConverter)
	if len(errs) > 0 {
...
func TestBidTransforms(t *testing.T) {
	...
	exchangeBidder := adaptLegacyAdapter(&mockAdapter)
	rateConverter := currencies.NewRateConverterDefault()
	seatBid, errs := exchangeBidder.requestBid(context.Background(), newAppOrtbRequest(), openrtb_ext.BidderRubicon, bidAdjustment, rateConverter)
	if len(errs) != 1 {
...
func TestWithTargeting(t *testing.T) {
	...
	exchangeBidder := adaptLegacyAdapter(&mockAdapter)
	rateConverter := currencies.NewRateConverterDefault()
	bid, errs := exchangeBidder.requestBid(context.Background(), ortbRequest, openrtb_ext.BidderFacebook, 1.0, rateConverter)
	if len(errs) != 0 {
