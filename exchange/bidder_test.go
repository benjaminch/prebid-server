...
func TestSingleBidder(t *testing.T) {
	...
	seatBid, errs := bidder.requestBid(context.Background(), &openrtb.BidRequest{}, "test", bidAdjustment, rateConverter)
...
func TestMultiBidder(t *testing.T) {
	...
	bidder := adaptBidder(bidderImpl, server.Client())
	rateConverter := currencies.NewRateConverterDefault()
	seatBid, errs := bidder.requestBid(context.Background(), &openrtb.BidRequest{}, "test", 1.0, rateConverter)
...
// TestTryGetConversionRate
func TestTryGetConversionRate(t *testing.T) {
	// TODO:
}
...
func TestMultiCurrencies_RateConverterNotSet(t *testing.T) {
	...
	testCases := []struct {
		...
		// Bidder respond with the same currency (not default one) on all HTTP responses
		{
			...
			expectedBidsCount: 0,
			expectedBadCurrencyErrors: []error{
				fmt.Errorf("Rates converter not set or inactive, cannot convert rates: 'EUR' => 'USD'"),
				fmt.Errorf("Rates converter not set or inactive, cannot convert rates: 'EUR' => 'USD'"),
				fmt.Errorf("Rates converter not set or inactive, cannot convert rates: 'EUR' => 'USD'"),
			},
		},
		...
		// Bidder responds with a mix of not set, non default currency and default currency in HTTP responses
		{
			...
			expectedBidsCount: 2,
			expectedBadCurrencyErrors: []error{
				fmt.Errorf("Rates converter not set or inactive, cannot convert rates: 'GDB' => 'USD'"),
			},
		},
...
func TestErrorReporting(t *testing.T) {
	...
	rateConverter := currencies.NewRateConverterDefault()
	bids, errs := bidder.requestBid(context.Background(), &openrtb.BidRequest{}, "test", 1.0, rateConverter)
	if bids != nil {
